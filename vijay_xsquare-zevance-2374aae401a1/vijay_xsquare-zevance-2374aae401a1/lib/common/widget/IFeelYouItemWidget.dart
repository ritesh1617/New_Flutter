...
class IFeelYouItemWidget extends StatelessWidget {
  FeelModel? model;
  String userId;
  final VoidCallback? onFollowClick;

  IFeelYouItemWidget(this.model, this.userId, {Key? key, this.onFollowClick})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    print("==========================${model!.user!.id!.toString()} ${userId}========================");
    return GestureDetector(
      onTap: () {
        if (model!.user != null) {
          if (model!.user!.id!.toString() == userId)
            navigatorKey.currentState!
                .pushNamed(RouteName.profileScreen, arguments: model!.user);
            else
          navigatorKey.currentState!
              .pushNamed(RouteName.publicProfileScreen, arguments: model!.user);
        }
      },
      child: Container(
